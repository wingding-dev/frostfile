...
The point is not to build a calendar inside FrostFile — it is to get these
...
def build_calendar(reminders: list[Reminder], *, stamp: datetime | None = None) -> str:
    ...
    lines = [
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
        "PRODID:-//FrostFile//Identity control reminders//EN",
        "CALSCALE:GREGORIAN",
        "METHOD:PUBLISH",
        "X-WR-CALNAME:FrostFile",
    ]
    ...
    for reminder in reminders:
        ...
        lines.append(f"UID:frostfile-{reminder.id}@localhost")
