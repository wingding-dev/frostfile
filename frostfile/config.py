...
Everything FrostFile persists lives in one directory so that "back up your
...
def default_data_dir() -> Path:
    ...
    override = os.environ.get("FROSTFILE_DATA_DIR")
    if override:
...
class Settings:
    ...
    def db_path(self) -> Path:
        return self.data_dir / "frostfile.db"
...
def load_settings(
    *,
    ...
    if data_dir is None and not os.environ.get("FROSTFILE_DATA_DIR"):
        seen = {resolved_dir}
        while True:
    ...
    data_unreachable = followed_pointer and not (
        resolved_dir / "frostfile.db"
    ).exists()

    resolved_host = host or os.environ.get("FROSTFILE_HOST", "127.0.0.1")
    resolved_port = port or int(os.environ.get("FROSTFILE_PORT", "8731"))
    try:
        timeout = int(
            os.environ.get("FROSTFILE_LOCK_MINUTES")
    ...
    if not settings.is_loopback and os.environ.get("FROSTFILE_ALLOW_REMOTE") != "1":
        raise SystemExit(
            f"Refusing to bind to {settings.host!r}.\n"
            "FrostFile holds Social Security numbers and freeze PINs, and has no\n"
            "transport encryption or multi-user access control. It is meant to be\n"
            "reachable only from the machine it runs on.\n"
            "If you truly need this, set FROSTFILE_ALLOW_REMOTE=1 and put it behind\n"
