...
def _maybe_auto_backup(request: Request, conn: sqlite3.Connection) -> None:
    ...
    used. Automatic copies are named frostfile-auto-* and only those are
...
def recovery_code_save(request: Request, csrf_token: str = Form(default="")):
    ...
    target = target_dir / "FrostFile-Recovery-Code.txt"
    contents = (
        "FrostFile Recovery Code\n"
        "========================\n\n"
        f"    {session.pending_recovery_code}\n\n"
        "If you ever forget your FrostFile passphrase: open FrostFile, click\n"
        ...
        "Anyone with this code AND your FrostFile data file can read your\n"
    ...
    return render(
        ...
        {"code": session.pending_recovery_code,
         "save_message": (
             ...
             "don't keep a copy of your FrostFile data file in the same "
             "account."
         )},
    )
...
async def setup_import(
    ...
    # Validate the file as a real FrostFile database before it replaces
    ...
    if not ok:
        ...
        return render(
            ...
            {
                "errors": [
                    "That file is not an FrostFile move file. On the old "
