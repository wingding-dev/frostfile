"""Command line entry point: `identilock`."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"identilock {__version__}")
    parser.add_argument(
    ...
    parser.add_argument(
        "--host",
        default=None,
        help="Interface to bind (default 127.0.0.1). Non-loopback needs IDENTILOCK_ALLOW_REMOTE=1.",
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"Identilock {__version__}")
...
def _run_app_window(server, url: str) -> bool:
    ...
    try:
        webview.create_window(
            "Identilock", url, width=1160, height=820, min_size=(760, 520)
        )
        webview.start()
    except Exception:
